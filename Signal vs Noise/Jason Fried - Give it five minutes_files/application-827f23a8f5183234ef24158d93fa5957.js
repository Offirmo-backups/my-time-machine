...
(function() {
  ...
  window.Attachment = {
    upload: function(form) {
      var frame;
      if (form.target !== ("" + form.id + "_target")) {
        frame = $("<iframe></iframe>");
        frame.attr("id", "" + form.id + "_target");
        frame.attr("name", "" + form.id + "_target");
        frame.attr("style", "width: 0; height; 0");
        frame.attr("src", "");
        frame.attr("frameborder", "0");
        frame.attr("onload", "Attachment.finishedUpload(this)");
        $("body").append(frame);
        $(form).attr("target", "" + form.id + "_target");
        form.submit();
      }
      return false;
    },
    finishedUpload: function(frame) {
      var response;
      console.log(frame);
      if (frame.contentDocument.body.firstChild) {
        response = frame.contentDocument.body.firstChild.textContent;
        return eval(response);
      }
    },
    "delete": function(attachment, attachable, attachable_type) {
      var confirmed;
      if (attachable_type == null) attachable_type = "Post";
      confirmed = confirm("Are you sure you want to delete this comment?");
      if (confirmed) {
        return $.ajax({
          url: "/admin/attachments/" + attachment + "?attachable_type=" + attachable_type + "&attachable_id=" + attachable,
          type: "DELETE",
          success: function() {
            return $("li#attachment_" + attachment).remove();
          }
        });
      }
    }
...
Autosave = function(input) {
  generateStorageKey = function(input) {
    return ["autosave:",  input.name, $(input).parent("form").action, escape(window.location.pathname)].join('');
  };
  ...
  watchForChanges = function() {
    $(document).bind('change', function() {self.save()});
    $(document).bind('paste', function() {self.save()});
    $(document).bind('keyup', function() {self.save()});
  };
  ...
  clearWhenSubmit = function() {
    $(self.input).parent("form").bind('submit', function() {
      self.clear();
    });
  };
...
(function() {
  ...
  window.Beanstalk = {
    ...
    beaconURL: function() {
      var host, url;
      host = Beanstalk.dev ? "dash.dev" : "beanstalk.37signals.com";
...
(function() {
  ...
  $(function() {
    if (document.location.hash.match(/#.*comment|#comments/)) {
      $('[data-comments]').show();
      $('[data-expandable]').removeClass("post-comments-info-expandable").addClass("expanded");
      return $('[data-comments-form]').show();
    }
  });

  window.NewComment = {
    preview: function() {
      $('[data-add-comment] form').hide();
      return $.ajax({
        url: "" + document.location.pathname + "/comments/preview",
        type: "POST",
        data: $("[data-add-comment] form").first().serialize(),
    ...
    edit: function() {
      $("[data-add-status]").html('Post');
      $("[data-add-comment] form").show();
      return $("[data-comment-preview]").html("");
    },
    post: function() {
      $("[data-comment-preview]").html("");
      return $.ajax({
        url: "" + (document.location.pathname.replace("/admin/", "/")) + "/comments",
        type: "POST",
        data: $("[data-add-comment] form").first().serialize(),
        success: function(response) {
          return eval(response);
  ...
  window.Comment = {
    "delete": function(comment, post) {
      var confirmed;
      confirmed = confirm("Are you sure you want to delete this comment?");
      if (confirmed) {
        return $.ajax({
          url: "/admin/posts/" + post + "/comments/" + comment,
          type: "DELETE",
          success: function() {
            return $("div#comment_" + comment).fadeOut();
          }
        });
      }
    },
    spam: function(comment, post) {
      var confirmed;
      confirmed = confirm("Are you sure you want to mark this comment as spam and delete it?");
      if (confirmed) {
        return $.ajax({
          url: "/admin/posts/" + post + "/comments/" + comment + "?spam=true",
          type: "DELETE",
          success: function() {
            return $("div#comment_" + comment).remove();
          }
        });
      }
    }
...
$(document).ready(function() {
  var zoomable = $('div.post-content figure');
  zoomable.click(function() {
    $(this).toggleClass("zoomed");
  });
});
/* Modernizr 2.6.1 (Custom Build) | MIT & BSD
 * Build: http://modernizr.com/download/#-fontface-backgroundsize-borderimage-borderradius-boxshadow-flexbox-flexbox_legacy-hsla-multiplebgs-opacity-rgba-textshadow-cssanimations-csscolumns-generatedcontent-cssgradients-cssreflections-csstransforms-csstransforms3d-csstransitions-hashchange-input-inputtypes-shiv-cssclasses-teststyles-testprop-testallprops-hasevent-prefixes-domprefixes-load
 */
...
(function() {
  ...
  social = {
    ...
    twitter: {
      url: function(url) {
        return "//cdn.api.twitter.com/1/urls/count.json?url=" + url + "&callback=?";
      },
      extract: function(_arg) {
        var count;
        count = _arg.count;
    ...
    linkedin: {
      url: function(url) {
        return "//www.linkedin.com/countserv/count/share?url=" + url + "&callback=?&format=jsonp";
      },
      extract: function(_arg) {
        var count;
        count = _arg.count;
  ...
  update = function($element, url) {
    var service;
    service = social[$element.data("share-counter")];
    return $.getJSON(service.url(url), function(response) {
      var count;
      count = service.extract(response);
      if (count > 0) return $element.text(" (" + count + ")");
    });
  };

  $(function() {
    var url;
    url = $("[data-page-url]").data("page-url");
    return $("[data-share-counter]").each(function() {
...
(function() {
  ...
  window.StarManager = {
    toggle: function(element, url) {
      var method, star;
      star = $(element).parent("li.star");
      method = star.hasClass("starred") ? "DELETE" : "PUT";
      return $.ajax(url, {
        type: method,
        success: function() {
          return star.toggleClass("starred");
        }
      });
    }
  };
...
Beanstalk.beaconURL = function() {
  return 'https://beanstalk.37signals.com/basecamp.gif?';
};
